...
- For a one-off artifact or result, use the registered `pixverse` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi pixverse <operation> --help
curl --fail --location https://runapi.ai/docs/api/pixverse/<operation>.md --output contract.md
```
...
```shell
runapi pixverse <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service pixverse --action <operation> > result.json
```
